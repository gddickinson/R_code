...
data_experimental <- data.frame(fish,bird,insects,bats,amphibians,mouse_rat,plants,fishTracking,birdTracking,birdPlumage,trapping,tracking,physiology,fishPhysiology,waterChemistry,soilChemistry,gis,hatchery,populationStats,genetic,restoration,photography,predation,breeding,transport,biologicalMeasures,survey)
...
par(new=T)
plot(x=data_experimental$mouse_rat+0.001,y=1:99, col= 1:99, axes = FALSE, labels = FALSE, xlab = "", ylab = "",log = "x", pch = 4)
par(new=T)
...
legend('bottomright',c("fish","birds","insects","bats","amphibians","mouse_rat","plants","fishTracking","birdTracking","birdPlumage","trapping","tracking","physiology","fishPhysiology","waterChemistry","soilChemistry","gis","hatchery","populationStats","genetic","restoration","photography","predation","breeding","transport","biologicalMeasures","survey"), pch = c(0,1,2,3,4), cex = 0.7)
par(new=F)
#par(opar)
##############################################
...
mouse_rat_n <- length(c("mouse","rat","sigmodon","rodent"))

data.wordNumber <- data.frame(fish_n,bird_n,insects_n,bats_n,amphibians_n,mouse_rat_nplants_n,fishTracking_n,birdTracking_n,birdPlumage_n,trapping_n,tracking_n,physiology_n,fishPhysiology_n,waterChemistry_n,soilChemistry_n,gis_n,hatchery_n,populationStats_n,genetic_n,restoration_n,photography_n,predation_n,breeding_n,transport_n,biologicalMeasures_n,survey_n)
...
fig_normalized<-barplot(data.normalized, main="text mining LCR pdfs (99 project groupings)", horiz=TRUE,cex.names=0.8, xlim=c(0,0.035), cex.axis=0.8, xlab = "normalized mean frequency of search terms")
#arrows(y0=fig,x0=data.totals+data.error,x1=data.totals-data.error,angle=90,code=3,length=0.1)
par(opar)
